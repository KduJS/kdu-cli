...
const externalKduScopedPackages = {
  '@kdujs/test-utils': true,
  '@kdujs/eslint-config': true
}
const localPackageRE = /'(@kdujs\/(?:cli|eslint|babel)[\w-]+)': '\^([\w-.]+)'/g
...
async function syncDeps ({ local, version, skipPrompt }) {
  ...
  if (!local) {
    ...
    const packages = await globby(['packages/@kdujs/*/package.json'])
    const resolvedPackages = (await Promise.all(packages.filter(filePath => {
      ...
      for (const dep in deps) {
        if (dep.match(/^@kdujs/) && !externalKduScopedPackages[dep]) {
          continue
        }
        let local = deps[dep]
        if (local.charAt(0) !== '^') {
  ...
  const paths = await globby(['packages/@kdujs/**/*.js'])
  paths
    ...
    .forEach(filePath => {
      ...
      const localReplacer = makeReplacer(
        pkg => {
          try {
            ...
            if (!semver.prerelease(version) && pkg.includes('@kdujs/cli-plugin')) {
      ...
      const updated = fs.readFileSync(filePath, 'utf-8')
        // update @kdujs packages in this repo
        .replace(localPackageRE, localReplacer)
        // also update kdu, kdu-template-compiler, kdux, kdu-router
...
if (!process.env.KDU_CLI_RELEASE) {
