...
async function convertRules (config) {
  ...
  const eslint = new ESLint({
    useEslintrc: false,
    baseConfig: {
      extends: [require.resolve(`@kdujs/eslint-config-${config}`)]
    }
  })
...
exports.buildEditorConfig = async function buildEditorConfig () {
  ...
  const configList = fs.readdirSync(path.resolve(__dirname, '../packages/@kdujs/'))
    .map(name => {
  ...
  await Promise.all(configList.map(async config => {
    ...
    const templateDir = path.resolve(__dirname, `../packages/@kdujs/cli-plugin-eslint/generator/template/${config}`)
