...
const { semver, matchesPluginId } = require('@kdujs/cli-shared-utils')
...
class PluginAPI {
   ...
   * @param {Service} service - A kdu-cli-service instance.
   */
  ...
  assertVersion (range) {
    ...
    throw new Error(
      `Require @kdujs/cli-service "${range}", but was loaded with "${this.version}".`
    )
  }
   ...
   *
   * @param {string} id - Plugin id, can omit the (@kdujs/|kdu-|@scope/kdu)-cli-plugin- prefix
   * @return {boolean}
   */
  hasPlugin (id) {
  ...
  /**
   * Register a command that will become available as `kdu-cli-service [name]`.
   *
   * @param {string} name
   * @param {object} [opts]
   *   {
