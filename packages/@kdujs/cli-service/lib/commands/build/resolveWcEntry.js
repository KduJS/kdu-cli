...
const createElement = (prefix, component, file, isAsync) => {
  ...
  return isAsync
    ? `window.customElements.define('${kebabName}', wrap(Kdu, () => import('~root/${file}?shadow')))\n`
    : `import ${camelName} from '~root/${file}?shadow'\n` +
        `window.customElements.define('${kebabName}', wrap(Kdu, ${camelName}))\n`
}

exports.fileToComponentName = (prefix, file) => {
  const basename = path.basename(file).replace(/\.(jsx?|kdu)$/, '')
...
import '${resolveImportPath('kdu-style-loader/lib/addStylesShadow')}'
import '${resolveImportPath('@kdujs/kdu-loader-v15/lib/runtime/componentNormalizer')}'
