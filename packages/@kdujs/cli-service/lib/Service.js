...
module.exports = class Service {
  ...
  resolvePkg (inlinePkg, context = this.context) {
    if (inlinePkg) {
      ...
      if (pkg.kduPlugins && pkg.kduPlugins.resolveFrom) {
        this.pkgContext = path.resolve(context, pkg.kduPlugins.resolveFrom)
  ...
  init (mode = process.env.KDU_CLI_MODE) {
    if (this.initialized) {
      return
    }
    ...
    debug('kdu:project-config')(this.projectOptions)
  ...
  loadEnv (mode) {
    ...
    if (mode) {
      ...
      const shouldForceDefaultEnv = (
        ...
        !process.env.KDU_CLI_TEST_TESTING_ENV
      )
  ...
  resolvePlugins (inlinePlugins, useBuiltIn) {
    ...
    // Local plugins
    if (this.pkg.kduPlugins && this.pkg.kduPlugins.service) {
      const files = this.pkg.kduPlugins.service
      if (!Array.isArray(files)) {
        throw new Error(`Invalid type for option 'kduPlugins.service', expected 'array' but got ${typeof files}.`)
      }
  ...
  loadUserOptions () {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        error(`Error loading ${chalk.bold('kdu.config.js')}:`)
        throw e
      }
    }

    // package.kdu
    pkgConfig = this.pkg.kdu
    if (pkgConfig && typeof pkgConfig !== 'object') {
      error(
        `Error loading kdu-cli config in ${chalk.bold(`package.json`)}: ` +
        `the "kdu" field should be an object.`
      )
      pkgConfig = null
    }
    ...
    if (fileConfig) {
      if (pkgConfig) {
        warn(
          `"kdu" field in package.json ignored ` +
          `due to presence of ${chalk.bold('kdu.config.js')}.`
        )
        warn(
          `You should migrate it into ${chalk.bold('kdu.config.js')} ` +
