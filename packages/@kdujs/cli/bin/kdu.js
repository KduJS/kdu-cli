...
if (
  slash(process.cwd()).indexOf('/packages/test') > 0 && (
    fs.existsSync(path.resolve(process.cwd(), '../@kdujs')) ||
    fs.existsSync(path.resolve(process.cwd(), '../../@kdujs'))
  )
) {
  process.env.KDU_CLI_DEBUG = true
}
...
program
  .version(`@kdujs/cli ${require('../package').version}`)
  .usage('<command> [options]')

program
  .command('create <app-name>')
  .description('create a new project powered by kdu-cli-service')
...
program
  ...
  .description('inspect the webpack config in a project with kdu-cli-service')
  .option('--mode <mode>')
...
// program
//   .command('ui')
//   .description('start and open the kdu-cli ui')
...
//   .action((options) => {
//     checkNodeVersion('>=8.6', 'kdu ui')
//     require('../lib/ui')(options)
//   })

program
  .command('init <template> <app-name>')
  .description('generate a project from a remote template (legacy API, requires @kdujs/cli-init)')
  .option('-c, --clone', 'Use git clone when fetching remote template')
  .option('--offline', 'Use cached template')
  .action(() => {
    loadCommand('init', '@kdujs/cli-init')
  })
    ...
    require('envinfo').run(
      {
        ...
        npmPackages: '/**/{typescript,*kdu*,@kdujs/*/}',
        npmGlobalPackages: ['@kdujs/cli']
      },
      {
        showNotFound: true,
        duplicates: true,
...
program.on('--help', () => {
  console.log()
  console.log(`  Run ${chalk.cyan(`kdu <command> --help`)} for detailed usage of given command.`)
  console.log()
})
