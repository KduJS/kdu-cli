...
const { toShortPluginId, matchesPluginId } = require('@kdujs/cli-shared-utils')
const ConfigTransform = require('./ConfigTransform')

const logger = require('@kdujs/cli-shared-utils/lib/logger')
const logTypes = {
  log: logger.log,
  info: logger.info,
  done: logger.done,
...
module.exports = class Generator {
  constructor (context, {
    ...
    const cliService = plugins.find(p => p.id === '@kdujs/cli-service')
  ...
  extractConfigFiles (extractAll, checkExisting) {
    ...
    if (extractAll) {
      ...
      if (!process.env.KDU_CLI_TEST) {
        // by default, always extract kdu.config.js
        extract('kdu')
      }
  ...
  async resolveFiles () {
    ...
    debug('kdu:cli-files')(this.files)
  }

  hasPlugin (_id) {
    if (_id === 'router') _id = 'kdu-router'
    if (['kdu-router', 'kdux'].includes(_id)) {
      const pkg = this.pkg
