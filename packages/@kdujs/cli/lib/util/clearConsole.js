...
const { clearConsole, hasYarn, hasPnpm3OrLater } = require('@kdujs/cli-shared-utils')
...
exports.generateTitle = async function (checkUpdate) {
  ...
  let title = chalk.bold.blue(`Kdu CLI v${current}`)

  if (process.env.KDU_CLI_TEST) {
    title += ' ' + chalk.blue.bold('TEST')
  }
  if (process.env.KDU_CLI_DEBUG) {
  ...
  if (checkUpdate && !error && semver.gt(latest, current)) {
    if (process.env.KDU_CLI_API_MODE) {
