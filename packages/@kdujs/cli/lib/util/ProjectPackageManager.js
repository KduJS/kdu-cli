...
class PackageManager {
  constructor ({ context, forcePackageManager } = {}) {
    ...
    if (this.bin === 'npm') {
      ...
      if (semver.lt(npmVersion, MIN_SUPPORTED_NPM_VERSION)) {
        throw new Error(
          ...
          'It does not support some core functionalities of Kdu CLI.\n' +
    ...
    const resolveFrom = projectPkg && projectPkg.kduPlugins && projectPkg.kduPlugins.resolveFrom

    // Logically, `resolveFrom` and `context` are distinct fields.
    // But in Kdu CLI we only care about plugins.
  ...
  async getRegistry (scope) {
    ...
    } else if (!process.env.KDU_CLI_TEST && await shouldUseTaobao(this.bin)) {
  ...
  async runCommand (command, args) {
    ...
    // In the use case of Kdu CLI, when installing dependencies,
  ...
  async upgrade (packageName) {
    ...
    for (const packname of packageName.split(' ')) {
      ...
      if (
        isTestOrDebug &&
        (packname === '@kdujs/cli-service' || isOfficialPlugin(resolvePluginId(realname)))
      ) {
