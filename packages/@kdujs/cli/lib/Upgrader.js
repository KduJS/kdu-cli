...
module.exports = class Upgrader {
  ...
  async runMigrator (packageName, options) {
    ...
    if (!process.env.KDU_CLI_TEST && hasProjectGit(this.context)) {
  ...
  async getUpgradable (includeNext) {
    ...
    // get current deps
    // filter @kdujs/cli-service, @kdujs/cli-plugin-* & kdu-cli-plugin-*
    for (const depType of ['dependencies', 'devDependencies', 'optionalDependencies']) {
      for (const [name, range] of Object.entries(this.pkg[depType] || {})) {
        if (name !== '@kdujs/cli-service' && !isPlugin(name)) {
          continue
        }
        ...
        if (semver.lt(installed, latest)) {
          // always list @kdujs/cli-service as the first one
          // as it's depended by all other plugins
          if (name === '@kdujs/cli-service') {
  ...
  async checkForUpdates (includeNext) {
    ...
    for (const p of upgradable) {
      const fields = [
        ...
        p.wanted,
        p.latest,
        `kdu upgrade ${p.name}${includeNext ? ' --next' : ''}`
      ]
