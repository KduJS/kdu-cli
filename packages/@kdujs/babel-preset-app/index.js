...
const defaultPolyfills = [
  ...
  // as kdu-template-es2015-compiler 1.8+ compiles it to Object.assign() calls.
  'es.object.assign',
...
module.exports = (context, options = {}) => {
  const presets = []
  const plugins = []
  const defaultEntryFiles = JSON.parse(process.env.KDU_CLI_ENTRY_FILES || '[]')

  // Though in the kdu-cli repo, we only use the two environment variables
  ...
  if (!process.env.KDU_CLI_TEST && process.env.NODE_ENV === 'test') {
    // Both Jest & Mocha set NODE_ENV to 'test'.
    // And both requires the `node` target.
    process.env.KDU_CLI_BABEL_TARGET_NODE = 'true'
    ...
    process.env.KDU_CLI_BABEL_TRANSPILE_MODULES = 'true'
  }
  ...
  if (options.jsx !== false) {
    ...
    try {
      ...
      kduVersion = semver.major(Kdu.version)
    } catch (e) {}

    if (kduVersion === 2) {
      presets.push([require('@kdujs/babel-preset-jsx'), jsxOptions])
    } else if (kduVersion === 3) {
      plugins.push([require('@kdujs/babel-plugin-jsx'), jsxOptions])
    }
  }
  ...
  if (process.env.KDU_CLI_BABEL_TARGET_NODE) {
    // running tests in Node.js
    targets = { node: 'current' }
  } else if (process.env.KDU_CLI_BUILD_TARGET === 'wc' || process.env.KDU_CLI_BUILD_TARGET === 'wc-async') {
    // targeting browsers that at least support ES2015 classes
    targets = getWCTargets(targets)
  } else if (process.env.KDU_CLI_MODERN_BUILD) {
    // targeting browsers that at least support <script type="module">
    targets = getModuleTargets(targets)
  }

  // included-by-default polyfills. These are common polyfills that 3rd party
  // dependencies may rely on (e.g. Kdux relies on Promise), but since with
  // useBuiltIns: 'usage' we won't be running Babel on these deps, they need to
  // be force-included.
  let polyfills
  const buildTarget = process.env.KDU_CLI_BUILD_TARGET || 'app'
  if (
    buildTarget === 'app' &&
    useBuiltIns === 'usage' &&
    !process.env.KDU_CLI_BABEL_TARGET_NODE
  ) {
  ...
  if (process.env.KDU_CLI_BABEL_TRANSPILE_MODULES) {
  ...
  plugins.push([require('@babel/plugin-transform-runtime'), {
    ...
    useESModules: !process.env.KDU_CLI_BABEL_TRANSPILE_MODULES,
...
// a special flag to tell @kdujs/cli-plugin-babel to include @babel/runtime for transpilation
// otherwise the above `include` option won't take effect
process.env.KDU_CLI_TRANSPILE_BABEL_RUNTIME = true
