...
const Service = require('@kdujs/cli-service')
const { toPlugin, findExisting } = require('./lib/util')

const babelPlugin = toPlugin('@kdujs/cli-plugin-babel')
const eslintPlugin = toPlugin('@kdujs/cli-plugin-eslint')
...
function resolveEntry (entry, cmd) {
  ...
  if (!entry) {
    ...
    console.log(chalk.red(`Valid entry file should be one of: main.js, index.js, App.kdu or app.kdu.`))
