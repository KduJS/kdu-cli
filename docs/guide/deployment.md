...
If you are using Kdu Router in `history` mode, a simple static file server will fail. For example, if you used Kdu Router with a route for `/todos/42`, the dev server has been configured to respond to `localhost:3000/todos/42` properly, but a simple static server serving a production build will respond with a 404 instead.

To fix that, you will need to configure your production server to fallback to `index.html` for any requests that do not match a static file. The Kdu Router docs provide [configuration instructions for common server setups](https://kdujs-router.web.app/guide/essentials/history-mode.html).
...
1. Set correct `publicPath` in `kdu.config.js`.

    If you are deploying to `https://<USERNAME>.github.io/` or to a custom domain, you can omit `publicPath` as it defaults to `"/"`.

    If you are deploying to `https://<USERNAME>.github.io/<REPO>/`, (i.e. your repository is at `https://github.com/<USERNAME>/<REPO>`), set `publicPath` to `"/<REPO>/"`. For example, if your repo name is "my-project", your `kdu.config.js` should look like this:
...
1. Set correct `publicPath` in `kdu.config.js` as explained above.
...
pages: # the job must be named pages
  ...
  script:
    - npm ci
    - npm run build
    - mv public public-kdu # GitLab Pages hooks on the public folder
...
Typically, your static website will be hosted on https://yourUserName.gitlab.io/yourProjectName, so you will also want to create an initial `kdu.config.js` file to [update the `BASE_URL`](https://github.com/kdujs/kdu-cli/tree/main/docs/config#baseurl) value to match your project name (the [`CI_PROJECT_NAME` environment variable](https://docs.gitlab.com/ee/ci/variables/predefined_variables.html) contains this value):
...
Commit both the `.gitlab-ci.yml` and `kdu.config.js` files before pushing to your repository. A GitLab CI pipeline will be triggered: when successful, visit your project's `Settings > Pages` to see your website link, and click on it.
...
#### Use history mode on Kdu Router

In order to receive direct hits using `history mode` on Kdu Router, you need to redirect all traffic to the `/index.html` file.
...
If you are using [@kdujs/cli-plugin-pwa](https://kdujs-cli.web.app/core-plugins/pwa.html#kdu-cli-plugin-pwa) make sure to exclude the `_redirects` file from being cached by the service worker.
To do so, add the following to your `kdu.config.js`:
```javascript
// kdu.config.js file to be placed in the root of your repository
...
```
Checkout [workboxOptions](https://kdujs-cli.web.app/core-plugins/pwa.html#configuration) and [exclude](https://developers.google.com/web/tools/workbox/reference-docs/latest/module-workbox-webpack-plugin.InjectManifest#InjectManifest) for more.
...
1. Create a new Static Site on Render, and give Render’s GitHub app permission to access your Kdu repo.
...
In order to receive direct hits using history mode on Kdu Router, you need to add the following rewrite rule in the `Redirects/Rewrites` tab for your site.
...
2. It is possible to publish to a subfolder of the main repository, for instance if you want to have multiple websites. In that case, set correct `publicPath` in `kdu.config.js`.
...
4. Create a `nginx.conf` file in the root of your project
    ...
    The following is a simple `nginx` configuration that serves your kdu project on port `80`. The root `index.html` is served for `page not found` / `404` errors which allows us to use `pushState()` based routing.
