...
Some global configurations for `@kdujs/cli`, such as your preferred package manager and your locally saved presets, are stored in a JSON file named `.kdurc` in your home directory. You can edit this file directly with your editor of choice to change the saved options.

You can also use the `kdu config` command to inspect or modify the global CLI config.
...
`kdu.config.js` is an optional config file that will be automatically loaded by `@kdujs/cli-service` if it's present in your project root (next to `package.json`). You can also use the `kdu` field in `package.json`, but do note in that case you will be limited to JSON-compatible values only.
...
/**
 * @type {import('@kdujs/cli-service').ProjectOptions}
 */
module.exports = {
  // options...
}
...
Or, you can use the `defineConfig` helper from `@kdujs/cli-service`, which could provide better intellisense support:
...
Deprecated since Kdu CLI 3.3, please use [`publicPath`](#publicPath) instead.
...
- Default: `'/'`
  ...
  The base URL your application bundle will be deployed at (known as `baseUrl` before Kdu CLI 3.3). This is the equivalent of webpack's `output.publicPath`, but Kdu CLI also needs this value for other purposes, so you should **always use `publicPath` instead of modifying webpack `output.publicPath`**.

  By default, Kdu CLI assumes your app will be deployed at the root of a domain, e.g. `https://www.my-app.com/`. If your app is deployed at a sub-path, you will need to specify that sub-path using this option. For example, if your app is deployed at `https://www.foobar.com/my-app/`, set `publicPath` to `'/my-app/'`.
...
- Default: `'dist'`
  ...
  The directory where the production build files will be generated in when running `kdu-cli-service build`. Note the target directory contents will be removed before building (this behavior can be disabled by passing `--no-clean` when building).
...
- Default: `true`
  ...
  By default, generated static assets contains hashes in their filenames for better caching control. However, this requires the index HTML to be auto-generated by Kdu CLI. If you cannot make use of the index HTML generated by Kdu CLI, you can disable filename hashing by setting this option to `false`.
...
- Default: `undefined`
  ...
  ::: tip
  When building in multi-page mode, the webpack config will contain different plugins (there will be multiple instances of `html-webpack-plugin` and `preload-webpack-plugin`). Make sure to run `kdu inspect` if you are trying to modify the options for those plugins.
  :::
...
- Default: `'default'`
  ...
  Whether to perform lint-on-save during development using [eslint-loader](https://github.com/webpack-contrib/eslint-loader). This value is respected only when [`@kdu/cli-plugin-eslint`](https://github.com/kdujs/kdu-cli/tree/main/packages/%40kdujs/cli-plugin-eslint) is installed.
...
- Default: `false`
  ...
  Whether to use the build of Kdu core that includes the runtime compiler. Setting it to `true` will allow you to use the `template` option in Kdu components, but will incur around an extra 10kb payload for your app.
...
- Type: `string | Object`
  ...
  If your frontend app and the backend API server are not running on the same host, you will need to proxy API requests to the API server during development. This is configurable via the `devServer.proxy` option in `kdu.config.js`.
...
::: tip
Kdu CLI uses `babel.config.js` which is a new config format in Babel 7. Unlike `.babelrc` or the `babel` field in `package.json`, this config file does not use a file-location based resolution, and is applied consistently to any file under project root, including dependencies inside `node_modules`. It is recommended to always use `babel.config.js` instead of other formats in Kdu CLI projects.
:::

All Kdu CLI apps use `@kdujs/babel-preset-app`, which includes `babel-preset-env`, JSX support and optimized configuration for minimal bundle size overhead. See [its docs](https://github.com/kdujs/kdu-cli/tree/main/packages/%40kdujs/babel-preset-app) for details and preset options.
...
See [@kdujs/cli-plugin-eslint](https://github.com/kdujs/kdu-cli/tree/main/packages/%40kdujs/cli-plugin-eslint) for more details.

## TypeScript

TypeScript can be configured via `tsconfig.json`.

See [@kdujs/cli-plugin-typescript](https://github.com/kdujs/kdu-cli/tree/main/packages/%40kdujs/cli-plugin-typescript) for more details.
