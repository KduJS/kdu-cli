...
A simple CLI for scaffolding Kdu.js projects.
...
``` bash
$ kdu init <template-name> <project-name>
```
...
The above command pulls the template from [kdujs-templates/webpack](https://github.com/kdujs-templates/webpack), prompts for some information, and generates the project at `./my-project/`.

### kdu build

Use kdu-cli as a zero-configuration development tool for your Kdu apps and component, check out the [docs](/docs/build.md).

### Official Templates

The purpose of official Kdu project templates are to provide opinionated, battery-included development tooling setups so that users can get started with actual app code as fast as possible. However, these templates are un-opinionated in terms of how you structure your app code and what libraries you use in addition to Kdu.js.

All official project templates are repos in the [kdujs-templates organization](https://github.com/kdujs-templates). When a new template is added to the organization, you will be able to run `kdu init <template-name> <project-name>` to use that template. You can also run `kdu list` to see all available official templates.

Current available templates include:

- [webpack](https://github.com/kdujs-templates/webpack) - A full-featured Webpack + kdu-loader setup with hot reload, linting, testing & css extraction.

- [webpack-simple](https://github.com/kdujs-templates/webpack-simple) - A simple Webpack + kdu-loader setup for quick prototyping.

- [browserify](https://github.com/kdujs-templates/browserify) - A full-featured Browserify + kduify setup with hot-reload, linting & unit testing.

- [browserify-simple](https://github.com/kdujs-templates/browserify-simple) - A simple Browserify + kduify setup for quick prototyping.

- [pwa](https://github.com/kdujs-templates/pwa) - PWA template for kdu-cli based on the webpack template

- [simple](https://github.com/kdujs-templates/simple) - The simplest possible Kdu setup in a single HTML file

### Custom Templates

It's unlikely to make everyone happy with the official templates. You can simply fork an official template and then use it via `kdu-cli` with:
...
``` bash
kdu init ~/fs/path/to-custom-template my-project
```
...
``` json
{
  "skipInterpolation": "src/**/*.kdu"
}
```

#### Metalsmith

`kdu-cli` uses [metalsmith](https://github.com/segmentio/metalsmith) to generate the project.

You may customize the metalsmith builder created by kdu-cli to register custom plugins.
...
- `helpers`: some helpers you can use to log results.
  ...
  - `logger`: [the built-in kdu-cli logger](/lib/logger.js)
...
`kdu-cli` uses the tool [`download-git-repo`](https://github.com/flipxfx/download-git-repo) to download the official templates used. The `download-git-repo` tool allows you to indicate a specific branch for a given repository by providing the desired branch name after a pound sign (`#`).

The format needed for a specific official template is:

```
kdu init '<template-name>#<branch-name>' <project-name>
```

Example:

Installing the [`1.0` branch](https://github.com/kdujs-templates/webpack-simple/tree/1.0) of the webpack-simple kdu template:

```
kdu init 'webpack-simple#1.0' mynewproject
```
