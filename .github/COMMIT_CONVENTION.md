...
Appears under "Bug Fixes" header, `k-model` subheader, with a link to issue #28:

```
fix(k-model): handle events on blur
...
```
perf(core): improve kdom diffing by removing 'foo' option
...
The scope could be anything specifying place of the commit change. For example `core`, `compiler`, `ssr`, `k-model`, `transition` etc...
